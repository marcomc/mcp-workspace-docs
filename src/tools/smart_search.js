...
function searchInFile(file, query, matches, limit, repo) {
...
function searchRepo(repo, query, fileGlob, limit, config) {
  ...
  for (const file of files) {
    ...
    searchInFile(file, query, matches, limit, repo);
  }

  return matches;
}

function sortByRepoAndPath(matches) {
  const repoOrder = { docs: 0, code: 1 };
  return matches.sort((a, b) => {
    const repoDiff = (repoOrder[a.repo] ?? 99) - (repoOrder[b.repo] ?? 99);
...
export async function smartSearch({ params, config }) {
  ...
  return sortByRepoAndPath(matches);
}
